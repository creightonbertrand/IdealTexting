...
@section('content')
    ...
    <section class="wrapper-bottom-sec">
        <div class="p-30 clearfix">
            ...
            <a href="{{url('user/sms/create-sms-reply-template')}}" class="btn btn-success btn-sm pull-right"><i class="fa fa-plus"></i> {{language_data('Create Template',Auth::guard('client')->user()->lan_id)}}</a>
        </div>
                                ...
                                @foreach($sms_templates as $st)
                                    <tr>
                                        ...
                                        <td data-label="Actions">
                                            ...
                                            <a class="btn btn-success btn-xs" href="{{url('user/sms/manage-sms-reply-template/'.$st->id)}}" ><i class="fa fa-edit"></i> {{language_data('Manage',Auth::guard('client')->user()->lan_id)}}</a>
          ...
          $('.data-table').DataTable({
            ...
            $( "body" ).delegate( ".cdelete", "click",function (e) {
              ...
              bootbox.confirm("{!! language_data('Are you sure',Auth::guard('client')->user()->lan_id) !!} ?", function (result) {
                    if (result) {
                        var _url = $("#_url").val();
                        window.location.href = _url + "/user/sms/delete-sms-reply-template/" + id;
                    }
                });
            });
