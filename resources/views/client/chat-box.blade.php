        ...
        <div class="row">
            <div class="col-md-12 p-lr-0">
                <div class="page-title-area clearfix">
                    <h2 class="page-title pull-left">{{language_data('Chat SMS',Auth::guard('client')->user()->lan_id)}}</h2>
                </div>
            </div>
        </div>
        <!-- Left Sidebar -->
                ...
                <div class="sidebar-content d-lg-block">
                    <div class="chart-body chat-fixed-search">
                        <fieldset class="form-group position-relative">
                            <input type="text" class="form-control search-query" id="iconLeft4" placeholder="{{language_data('Search',Auth::guard('client')->user()->lan_id)}}">
                    ...
                    <div id="users-list" class="list-group position-relative sms_history">
                        <div class="users-list-padding member_list"  id="sms-history-data">
                            @include('client.get-chat-box')
                        </div>

                        @if ($sms_count > 15)
                            <div class="load_div">
                                <a href="#" class="btn btn-success btn-xs" id="load_more">{{language_data('Load More',Auth::guard('client')->user()->lan_id)}}</a>
                            </div>
                        @endif
                    ...
                    <form class="chat-app-input d-flex">
                        <div class="row">
                            ...
                            <form method="POST" action="#" id="ReplySMS">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <textarea class="form-control" id="msg" placeholder="{{language_data('Type your message',Auth::guard('client')->user()->lan_id)}}" name="message"></textarea>
                                        <div class="col-md-12">
                                            @foreach($sms_templates as $st)
                                                    <span class="badge-t badge-pill badge-primary" onclick="fillForm(`{{$st->message}}`)">{{$st->template_name}}</span>
                                            @endforeach
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    ...
                                    <a href="#" class="btn btn-complete blacklist-btn" data-toggle="tooltip" data-placement="top"
                                       title="{{language_data('Add To Blacklist',Auth::guard('client')->user()->lan_id)}}"> <i class="fa fa-user-md d-lg-none"></i>
                                    </a>

                                    <a href="#" class="btn btn-danger remove-btn" data-toggle="tooltip" data-placement="top"
                                       title="{{language_data('Remove History',Auth::guard('client')->user()->lan_id)}}"> <i class="fa fa-trash d-lg-none"></i>
                                    </a>
                                </div>
                            </form>
    ...
    <script>
        ...
        $(".search-query").on("keyup", function () {
            window.waitForFinalEvent(function () {
                ...
                var loadValue = 'usms_search_'+value;
                loadMoreData(loadValue);
                $(".users-list-padding li").filter(function () {
                    $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
                });

            }.bind(this), 800, "get_chat_box_history");
        });
        ...
        SMSHistory.delegate('li', 'click', function (e) {
            ...
            request = $.ajax({
                type: "POST",
                url: _url + "/user/sms/view-reports",
                data: {
                    id: id
                },
                cache: false
        ...
        $("form").submit(function (event) {
            ...
            request = $.ajax({
                url: _url + "/user/sms/reply-chat-sms",
                type: "post",
                data: serializedData
            });
        ...
        $(".blacklist-btn").on('click', function (event) {
            ...
            blacklist_request = $.ajax({
                type: "POST",
                url: _url + "/user/sms/add-to-blacklist",
                data: {
                    sms_id: sms_id
                },
                cache: false
        ...
        $(".remove-btn").on('click', function (event) {
            ...
            sms_history_request = $.ajax({
                type: "POST",
                url: _url + "/user/sms/remove-chat-history",
                data: {
                    sms_id: sms_id
                },
                cache: false
        ...
        $('#tags').tagsInput({
            'onAddTag':function(e) {
                ...
                sms_history_request = $.ajax({
                    type: "POST",
                    url: _url + "/user/sms/add-chat-tag",
                    data: {
                        sms_id: sms_id,
                        tag: e
                    },
            ...
            'onRemoveTag':function(e) {
                ...
                sms_history_request = $.ajax({
                    type: "POST",
                    url: _url + "/user/sms/remove-chat-tag",
                    data: {
                        sms_id: sms_id,
                        tag: e
                    },
