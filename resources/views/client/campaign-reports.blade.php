...
@section('content')
    ...
    <section class="wrapper-bottom-sec">
        <div class="p-30">
            <h2 class="page-title inline-block">{{language_data('Campaign Reports',Auth::guard('client')->user()->lan_id)}}</h2>
        </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <label for="camp_type">{{language_data('Campaign Type',Auth::guard('client')->user()->lan_id)}}</label>
                                            <select class="selectpicker form-control" name="camp_type" id="camp_type">
                                                ...
                                                <option value="regular">{{language_data('Regular',Auth::guard('client')->user()->lan_id)}}</option>
                                                <option value="scheduled">{{language_data('Scheduled',Auth::guard('client')->user()->lan_id)}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <label for="campaign_id">{{language_data('Campaign ID',Auth::guard('client')->user()->lan_id)}}</label>
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <label for="from">{{language_data('From',Auth::guard('client')->user()->lan_id)}}</label>
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <label for="status">{{language_data('Status',Auth::guard('client')->user()->lan_id)}}</label>
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <label for="date_from">{{language_data('Date',Auth::guard('client')->user()->lan_id)}} {{language_data('From')}}</label>
                                    ...
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <label for="date_to">{{language_data('Date',Auth::guard('client')->user()->lan_id)}} {{language_data('To',Auth::guard('client')->user()->lan_id)}}</label>
                ...
                <div class="col-sm-12">
                    <div class="panel">
                        <div class="panel-heading">
                            ...
                            <h3 class="panel-title">{{language_data('Campaign Reports',Auth::guard('client')->user()->lan_id)}}</h3>
                        </div>
                        <div class="panel-body">
                            ...
                            <table class="table data-table table-hover">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 20%;">{{language_data('From',Auth::guard('client')->user()->lan_id)}}</th>
                                    <th style="width: 10%;">{{language_data('Recipients',Auth::guard('client')->user()->lan_id)}}</th>
                                    <th style="width: 15%;">Throttling</th>
                                    <th style="width: 5%;">{{language_data('Status',Auth::guard('client')->user()->lan_id)}}</th>
                                    <th style="width: 35%;">{{language_data('Action',Auth::guard('client')->user()->lan_id)}}</th>
                                </tr>
                                </thead>
                            </table>
                        </div>
    ...
    <script>
        $(document).ready(function(){
            ...
            var oTable = $('.data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{!! url('user/sms/get-campaign-history/') !!}',
                    data: function (d) {
                ...
                columns: [
                    ...
                    {data: 'throttling', name: 'throttling'},
            ...
            deleteTriger.on("click", function(event){ // triggering delete one by one
                if( $('.deleteRow:checked').length > 0 ){  // at-least one checkbox checked
                    ...
                    $.ajax({
                        type: "POST",
                        url: '{!! url('user/sms/bulk-campaign-delete/') !!}',
            ...
            $( "body" ).delegate( ".cdelete", "click",function (e) {
                ...
                bootbox.confirm("{!! language_data('Are you sure') !!}?", function (result) {
                    if (result) {
                        var _url = $("#_url").val();
                        window.location.href = _url + "/user/sms/delete-campaign/" + id;
                    }
                });
            });

            /*For Pause SMS*/
            $( "body" ).delegate( ".cpause", "click",function (e) {
                e.preventDefault();
                var id = this.id;
                bootbox.confirm("{!! language_data('Are you sure') !!}?", function (result) {
                    if (result) {
                        var _url = $("#_url").val();
                        window.location.href = _url + "/user/sms/pause-campaign/" + id;
                    }
                });
            });

            /*For Delete SMS*/
            $( "body" ).delegate( ".cresume", "click",function (e) {
                e.preventDefault();
                var id = this.id;
                bootbox.confirm("{!! language_data('Are you sure') !!}?", function (result) {
                    if (result) {
                        var _url = $("#_url").val();
                        window.location.href = _url + "/user/sms/resume-campaign/" + id;
                    }
                });
            });
