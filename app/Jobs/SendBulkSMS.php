...
class SendBulkSMS implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($this->gateway->custom == 'Yes' && $this->cg_info != '' && $this->gateway->type == 'http') {
            ...
            if ($gateway_name == 'ConnectBindSMPP' || $gateway_name == 'CMTelecomSMPP') {
                ...
                try {
                    ...
                    $transport = new \SocketTransport([$gateway_url], $gateway_port);
            ...
            switch ($gateway_name) {
                    ...
                    try {
                        ...
                        $get_response = $client->messages->create(
                                $phone, [
                                        'from'           => $sender_id,
                                        'body'           => $this->message,
                                        'statusCallback' => url('sms/delivery-report-twilio'),
                                ]
                        );
                ...
                case 'Route SMS':
                    ...
                    $cl_phone          = urlencode(trim($this->cl_phone));
                    $sms_url           = rtrim($gateway_url, '/');
                    ...
                    $sms_sent_to_user = "$sms_url"."/bulksms/bulksms?type=$type"."&username=$gateway_user_name"."&password=$gateway_password"."&destination=$cl_phone"."&source=$sender_id"."&message=$message"."&dlr=1";
                ...
                case 'Zang':
                    ...
                    curl_setopt($ch, CURLOPT_URL,
                            "https://api.zang.io/v2/Accounts/{$gateway_user_name}/SMS/Messages.json");
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                    curl_setopt($ch, CURLOPT_POSTFIELDS,
                            "To=$this->cl_phone&From=$this->sender_id&Body=$this->message");
                ...
                case 'InfoBip':
                    ...
                    $destination = ["messageId" => $message_id, "to" => $this->cl_phone];
                    $message     = [
                            "from"              => $this->sender_id,
                ...
                case 'Plivo':
                    ...
                    $post_field = [
                            ...
                            'dst'  => $clphone,
                            'text' => $this->message,
                            'url'  => url('sms/delivery-report-plivo'),
                    ];
                    ...
                    if (json_last_error() == JSON_ERROR_NONE) {
                        if (isset($response) && is_array($response) && array_key_exists('message', $response)) {
                            $get_sms_status = $response['message'].'|'.$response['message_uuid'][0];
                ...
                case 'Text Local':
                    ...
                    $data = ['apikey' => $apiKey, 'numbers' => $clphone, "sender" => $sender];
                ...
                case 'Hablame':
                    ...
                    $clphone     = str_replace('+', '', $clphone);
                    $data        = [
                            'cliente' => $gateway_user_name,
                        //Numero de cliente
                            'api'     => $gateway_password,
                        //Clave API suministrada
                            'numero'  => $clphone,
                        //numero o numeros telefonicos a enviar el SMS (separados por una coma ,)
                            'sms'     => $this->message,
                ...
                case 'Wavecell':
                    ...
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                    curl_setopt($ch, CURLOPT_POSTFIELDS,
                            "{ \"source\":\"$this->sender_id\", \"destination\":\"$this->cl_phone\", \"text\":\"$this->message\", \"encoding\":\"AUTO\" }");
                    curl_setopt($ch, CURLOPT_POST, 1);
                ...
                case 'MLat':
                    ...
                    try {
                        ...
                        $mlat           = new \SoapClient($gateway_url.'?wsdl',
                                ['location' => 'https://m-lat.net/axis2/services/SMSServiceWS?wsdl']);
                        $credential     = ['user' => $gateway_user_name, 'password' => $gateway_password];
                        $get_sms_status = $mlat->sendManyTextSMS([
                ...
                case 'Orange':
                    ...
                    if ( ! empty($response['access_token'])) {
                        ...
                        if ( ! empty($gateway_extra)) {
                            $args = [
                                    'outboundSMSMessageRequest' => [
                                            'address'                => $receiverAddress,
                                            'senderAddress'          => $senderAddress,
                                            'senderName'             => urlencode($gateway_extra),
                        ...
                        if (curl_errno($ch)) {
                            ...
                            if (isset($get_data) && is_array($get_data) && array_key_exists('requestError',
                                            $get_data)) {
                ...
                case 'Itexmo':
                    ...
                    $ch     = curl_init();
                    $itexmo = ['1' => $this->cl_phone, '2' => $this->message, '3' => $gateway_user_name];
                    curl_setopt($ch, CURLOPT_URL, $gateway_url);
                    curl_setopt($ch, CURLOPT_POST, 1);
                    curl_setopt($ch, CURLOPT_POSTFIELDS,
                ...
                case 'SendOut':
                    ...
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                    curl_setopt($ch, CURLOPT_POSTFIELDS,
                            "{\"api_id\": \"$gateway_user_name\",\"api_token\": \"$gateway_password\" ,\"debug\": \"true\",\"to\": [\"$clphone\"],\"sms\": \"$this->message\"} ");
                    curl_setopt($ch, CURLOPT_POST, 1);
                ...
                case 'MessageBird':
                    ...
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                    curl_setopt($ch, CURLOPT_POSTFIELDS,
                            "recipients=$this->cl_phone&originator=$this->sender_id&body=$this->message&datacoding=auto");
                    curl_setopt($ch, CURLOPT_POST, 1);
                ...
                case 'LightSMS':
                    ...
                    try {
                        ...
                        if (is_array($result) && array_key_exists($clphone, $result)) {
                            ...
                            if ( ! empty($result[$clphone]) && is_array($result[$clphone]) && array_key_exists('error',
                                            $result[$clphone])) {
                ...
                case 'TeleSign':
                    ...
                    if (is_array($get_status) && array_key_exists('status', $get_status)) {
                        if (is_array($get_status['status']) && array_key_exists('description',
                                        $get_status['status']) && array_key_exists('code', $get_status['status'])) {
                ...
                case 'Digitalreachapi':
                    ...
                    // DATA JASON ENCODED
                    $data      = ["u_name" => $gateway_user_name, "passwd" => $gateway_password];
                ...
                case 'BSG':
                    ...
                    curl_setopt($ch, CURLOPT_HTTPHEADER,
                            ['Content-Type: application/json', "X-API-KEY:$gateway_user_name"]);
                    curl_setopt($ch, CURLOPT_POST, 1);
                    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                    ...
                    {\n \"from\": \"$sender_id\",\n  \"to\": [\n \"$clphone\" \n],\n  \"body\": \"$this->message\"\n  }");
                        ...
                        if (is_array($result) && array_key_exists('id', $result)) {
                            ...
                            curl_setopt($curl, CURLOPT_URL,
                                    $gateway_url."/batches/".$batch_id."/delivery_report/".$recipient);
                ...
                case 'Broadnet':
                    ...
                    try {
                        ...
                        if (strpos($get_sms_status, 'Response') !== false) {
                            ...
                            if (isset($response) && is_array($response) && array_key_exists(1, $response)) {
                ...
                case 'Mailjet':
                    ...
                    try {
                        ...
                        if ($response && is_array($response)) {
                            if (array_key_exists('Status', $response) && array_key_exists('Code',
                                            $response['Status'])) {
                                $get_sms_status = 'Success';
                ...
                case 'Ejoin':
                    ...
                    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                    curl_setopt($ch, CURLOPT_URL,
                            $gateway_url."?username=".$gateway_user_name."&password=".$gateway_password);
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                    curl_setopt($ch, CURLOPT_POST, 1);
                    ...
                    if (curl_error($ch)) {
                        ...
                        if (isset($get_response) && is_array($get_response) && array_key_exists('code',
                                        $get_response) && array_key_exists('reason', $get_response)) {
                ...
                case 'Mobitel':
                    ...
                    try {
                        ...
                        $get_data = sendMessages($session, $this->sender_id, $this->message, [$clphone], $sms_type);
                        $get_data = trim($get_data);
                ...
                case 'Ayyildiz':
                    ...
                    $clphone    = str_replace([" ", '+'], "", $this->cl_phone);
                    $send_sms   = new \sendSMS($gateway_user_name, $gateway_password, $gateway_extra, $this->sender_id,
                            $clphone, $this->message);
                ...
                case 'Diamondcard':
                    ...
                    try {
                        ...
                        $params = [
                                'AccId'       => $gateway_user_name,                   // Account Id
                        ...
                        $response = $client->call('send', ['inParams' => $params]);
                ...
                case 'EblogUs':
                    ...
                    try {
                        ...
                        if (isset($msg_result) && is_array($msg_result) && array_key_exists('message_id',
                                        $msg_result)) {
                ...
                case 'Connectmedia':
                    ...
                    $post     = [
                            ...
                            'to'       => [$this->cl_phone], //Please don’t change
                ...
                case 'ClickSend':
                    ...
                    try {
                        ...
                        curl_setopt($ch, CURLOPT_POST, true);
                        curl_setopt($ch, CURLOPT_POSTFIELDS,
                                "username=$gateway_user_name&key=$gateway_password&to=$clphone&message=$message&senderid=$this->sender_id");
                ...
                case 'GhBulkSms':
                    ...
                    try {
                        ...
                        $sms_sent_to_user = $gateway_url."?option=com_spc&comm=spc_api&username=$gateway_user_name"."&password=$gateway_password"."&recipient=$clphone"."&message=$message"."&sender=$this->sender_id";
                ...
                case 'Ovh':
                    ...
                    if ($this->sender_id != '') {
                        ...
                        if (isset($senders) && is_array($senders) && count($senders) > 0 && in_array($this->sender_id,
                                        $senders)) {
                ...
                case 'Signalwire':
                        $space_url = preg_replace("(^https?://)", "", rtrim($this->gateway->api_link, '/'));
                        try {
                            $client = new \SignalWire\Rest\Client($gateway_user_name, $gateway_password, array("signalwireSpaceUrl" => $space_url));
                            ...
                            $get_response = $client->messages->create(
                                $phone, array(
                                    'from' => $sender_id,
                                    'body' => $this->message,
                                    'statusCallback' => url('sms/delivery-report-signalwire'),
                                )
                            );
                            ...
                            $get_sms_status = 'Failed'.'|'.$e->getMessage();
                        }
                    ...
                    if ($result === false) {
                        ...
                        if (isset($json_result) && is_array($json_result)) {
                            ...
                            if (array_key_exists('messages',
                                            $json_result) && isset($json_result['messages']['0']['status'])) {
                ...
                case 'Telnyx':
                    ...
                    try {
                        ...
                        if (isset($get_response) && is_array($get_response)) {
                            if (array_key_exists('data', $get_response) && array_key_exists('to',
                                            $get_response['data']) && $get_response['data']['to'][0]['status'] == 'queued') {
                                $get_sms_status = 'Success';
                ...
                case 'Trumpia':
                    ...
                    try {
                        ...
                        if (isset($get_response) && is_array($get_response) && array_key_exists('status_code',
                                        $get_response)) {
                ...
                case 'FortyTwo':
                    ...
                    if ($err) {
                        ...
                        if (isset($response) && is_array($response) && array_key_exists('result_info', $response)) {
                            if (is_array($response['result_info']) && array_key_exists('status_code',
                                            $response['result_info']) && array_key_exists('description',
                ...
                case '019SMS':
                    ...
                    curl_setopt($CR, CURLOPT_HTTPHEADER, ["charset=utf-8"]);
                    $result = curl_exec($CR);
                    $error  = curl_error($CR);
                ...
                case 'GoIP':
                    ...
                    try {
                        $get_sms_status = file_get_contents($gateway_url."/default/en_US/send.html?u=$gateway_user_name&p=$gateway_password&l=1&n=$this->cl_phone&m=$message",
                                false, $context);
                ...
                case 'AfricasTalking':
                    ...
                    try {
                        ...
                        if (isset($get_data) && is_array($get_data) && array_key_exists('data',
                                        $get_data) && array_key_exists('status',
                                        $get_data) && isset($get_data['data']->SMSMessageData)) {
                    ...
                    try {
                        ...
                        switch ($http_code) {
                            case '200':
                                ...
                                if (isset($get_data) && is_array($get_data) && array_key_exists('result_info',
                                                $get_data)) {
                ...
                case 'Ytel':
                    ...
                    try {
                        ...
                        curl_setopt_array($curl, [
                                ...
                                CURLOPT_HTTP_VERSION   => CURL_HTTP_VERSION_1_1,
                                CURLOPT_CUSTOMREQUEST  => "POST",
                                CURLOPT_POSTFIELDS     => "From=$this->sender_id&To=$this->cl_phone&Body=".urlencode($this->message),
                                CURLOPT_HTTPHEADER     => [
                        ...
                        if (isset($get_data) && is_array($get_data) && array_key_exists('Message360', $get_data)) {
                            if (array_key_exists('Errors', $get_data['Message360'])) {
                                $get_sms_status = $get_data['Message360']['Errors']['Error'][0]['Message'];
                            } elseif (array_key_exists('Message', $get_data['Message360'])) {
                                $get_sms_status = 'Success|'.$get_data['Message360']['Message']['MessageSid'];
                            } else {
                                $get_sms_status = (string) $response;
                            }
                        } elseif (array_key_exists('message', $get_data)) {
                            $get_sms_status = $get_data['message'];
                        } else {
                            $get_sms_status = (string) $response;
                        }
                ...
                case 'SpeedsmsVN':
                    ...
                    if (curl_error($ch)) {
                        ...
                        if (isset($get_sms_status) && is_array($get_sms_status) && array_key_exists('status',
                                        $get_sms_status)) {
                ...
                case 'Safaricom':
                    ...
                    $auth_url = $gateway_url.'/api/auth/login';
                    ...
                    if ($err) {
                        ...
                        if (isset($response) && is_array($response) && array_key_exists('token', $response)) {
                            ...
                            $send_sms_url = $gateway_url.'/api/public/CMS/bulksms';
                            ...
                            $data = [
                                    'timeStamp' => date('Ymd'),
                                    'dataSet'   => [
                                            'userName'     => "FireflySmartify",
                                            'channel'      => 'sms',
                                            'packageId'    => $gateway_extra,
                                            'campaignType' => $gateway_port,
                                            'oa'           => $this->sender_id,
                                            'msisdn'       => $this->cl_phone,
                                            'message'      => $this->message,
                ...
                case 'SMSEdgeIO':
                    ...
                    if (isset($get_data) && is_array($get_data) && array_key_exists('0', $get_data)) {
                        if (isset($get_data[0]) && is_array($get_data[0]) && array_key_exists('success',
                                        $get_data[0])) {
                            if ($get_data[0]['success']) {
                                $get_sms_status = 'Success|'.$get_data[0]['data']['id'];
                            } else {
                    ...
                    try {
                        ...
                        if (curl_error($ch)) {
                            ...
                            if ($response && is_array($response)) {
                                ...
                                if ($this->msgcount > 1) {
                                    if (isset($response[0]) && is_array($response[0]) && array_key_exists('message_id',
                                                    $response[0])) {
                                        $get_sms_status = 'Success';
                                    } elseif (array_key_exists('error_message', $response)) {
                                        $get_sms_status = $response['error_message'];
                ...
                case 'RawMobility':
                    ...
                    try {
                        ...
                        if (curl_errno($ch)) {
                            ...
                            if (isset($output) && is_array($output) && array_key_exists('detailedResponse',
                                            $output) && array_key_exists('routeId', $output)) {
                ...
                case 'AdnSMS':
                    ...
                    try {
                        ...
                        if ($curl_error) {
                            ...
                            if (isset($get_sms_status) && is_array($get_sms_status) && array_key_exists('api_response_message',
                                            $get_sms_status)) {
                ...
                case 'Thinq':
                    ...
                    if ($curl_error) {
                        ...
                        if ($response && is_array($response)) {
                            ...
                            if ((array_key_exists('code', $response) && array_key_exists('message',
                                                    $response) && $response['code'] == 200) || array_key_exists('guid',
                                            $response)) {
                                $get_sms_status = 'Success';
                            } else {
                                $get_sms_status = $response['message'];
                    ...
                    try {
                        ...
                        if (isset($result) && is_array($result)) {
                            if (array_key_exists('result',
                                            $result) && is_array($result['result']) && array_key_exists('0',
                                            $result['result'])) {
                            ...
                            } elseif (array_key_exists('error',
                                            $result) && is_array($result['error']) && array_key_exists('description',
                                            $result['error'])) {
                ...
                case 'Ebulksms':
                    ...
                    try {
                        ...
                        if ($response && is_array($response) && array_key_exists('response',
                                        $response) && array_key_exists('status', $response['response'])) {
                ...
                case 'SpryngSMS':
                    ...
                    if (curl_errno($ch)) {
                        ...
                        if (is_array($get_data)) {
                            if (array_key_exists('id', $get_data)) {
                                $get_sms_status = 'Success';
                            } elseif (array_key_exists('error', $get_data)) {
                ...
                case 'PaaSoo':
                    ...
                    if (curl_errno($ch)) {
                        ...
                        if (isset($output) && is_array($output) && array_key_exists('status', $output)) {
                            if ($output['status'] == '0') {
                                $get_sms_status = 'Success';
                            } else {
                                $get_sms_status = $output['status_code'];
                            }
                        } else {
                            $get_sms_status = 'Invalid Request';
                        }
                ...
                case 'DidoSystem':
                    ...
                    if (curl_errno($ch)) {
                        ...
                        if (isset($output) && is_array($output) && array_key_exists('success', $output)) {
                            if ($output['success'] == true) {
                                $get_sms_status = 'Success';
                            } else {
                                $get_sms_status = $output['error']['message'];
                            }
                        } else {
                            $get_sms_status = 'Invalid Request';
                        }
                ...
                case 'SMSEASYSMS':
                    ...
                    if (curl_error($ch)) {
                        ...
                        $get_response = json_decode($get_data, true);
                        if (isset($get_response) && is_array($get_response)) {
                            if (isset($get_response[0]) && is_array($get_response[0]) && array_key_exists('accepted',
                                            $get_response[0]) && $get_response[0]['accepted'] == true) {
                                $get_sms_status = 'Success';
                            } elseif (array_key_exists('error', $get_response)) {
                                $get_sms_status = $get_response['error']['description'];
                            } else {
                                $get_sms_status = (string) $get_data;
                            }
                        } else {
                ...
                case 'ClockWiseSMS':
                    ...
                    curl_setopt($ch, CURLOPT_URL, $gateway_url);
                    curl_setopt($ch, CURLOPT_HTTPHEADER, ["Content-Type: application/json"]);
                    curl_setopt($ch, CURLOPT_HTTPHEADER, ["x-api-key: ".$gateway_user_name]);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($json));
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    $get_sms_status = curl_exec($ch);
                    ...
                    if (curl_errno($ch)) {
                        ...
                        if (isset($output) && is_array($output)) {
                            if (array_key_exists('ids', $output)) {
                                $get_sms_status = 'Success';
                            } elseif (array_key_exists('error', $output)) {
                                $get_sms_status = $output['error']['message'];
                            } else {
                ...
                case 'BulkSmsPlanet':
                    ...
                    if (strpos($get_sms_status, 'SMS-SHOOT-ID') !== false) {
                        ...
                        $get_data = json_decode($get_sms_status, true);
                        if (isset($get_data) && array_key_exists('message', $get_data)) {
                            $get_sms_status = $get_data['message'];
                        }
                    }
                    break;
                ...
                case 'BulkSMSOnline':
                    ...
                    if (curl_errno($ch)) {
                        ...
                        if (strpos($get_sms_status, 'OK') !== false) {
                            $get_sms_status = 'Success|'.str_replace('OK: ', '', $get_sms_status);
                        } else {
                ...
                case 'SkylineLabsCC':
                    ...
                    $timezone = +8;
                    $time     = gmdate("YmdHis", time() + 3600 * ($timezone + date("I")));
                    $sign     = md5($gateway_user_name.$gateway_password.$time);

                    $parameters = [
                            'account'  => $gateway_user_name,
                            'sign'     => $sign,
                            'numbers'  => $this->cl_phone,
                            'content'  => $this->message,
                            'datetime' => $time,
                    ];

                    if ($this->sender_id) {
                        $parameters['senderID'] = $this->sender_id;
                    }
                ...
                case 'SempicoSolutions':
                    ...
                    $ch      = curl_init();
                    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                    curl_setopt($ch, CURLOPT_URL, 'https://restapi.sempico.solutions/v1/authorize');
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                    curl_setopt($ch, CURLOPT_POST, 1);
                    ...
                    if (isset($get_response) && is_array($get_response) && array_key_exists('status', $get_response)) {
                        if ($get_response['status'] == false) {
                            $get_sms_status = $get_response['errors']['password'][0];
                        } else {
                            $authorization_code = $get_response['data']['authorization_code'];
                            ...
                            $ch = curl_init();
                            curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                            curl_setopt($ch, CURLOPT_URL, 'https://restapi.sempico.solutions/v1/accesstoken');
                            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
                            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                            curl_setopt($ch, CURLOPT_POST, 1);
                            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($code_data));
                            $get_data       = curl_exec($ch);
                            $get_sms_status = curl_error($ch);
                            curl_close($ch);
                            ...
                            if (isset($get_response) && is_array($get_response) && array_key_exists('status', $get_response)) {
                                if ($get_response['status'] == false) {
                                    ...
                                    $access_token = $get_response['data']['access_token'];
                                    ...
                                    if (isset($get_response) && is_array($get_response) && array_key_exists('status', $get_response)) {
                                        if ($get_response['status'] == true) {
                                            $get_sms_status = 'Success';
                                        } elseif ($get_response['status'] == false) {
                                            $get_sms_status = $get_response['errors'];
                                        } else {
                                            $get_sms_status = $get_response['message'];
                                        }
                                    } else {
                ...
                case 'Octopush':
                    ...
                    $postFields = [
                            "user_login"     => $gateway_user_name,
                            "api_key"        => $gateway_password,
                            "sms_type"       => 'FR',
                            "sms_recipients" => $this->cl_phone,
                            "sms_sender"     => $this->sender_id,
                    ...
                    if (isset($get_response) && is_array($get_response) && array_key_exists('error_code', $get_response)) {
                        if ($get_response['error_code'] == '000') {
                            $get_sms_status = 'Success';
                        } else {
                            $get_sms_status = $get_response['error_code'];
                        }
                    }
                    break;
                ...
                case 'WinSMS':
                    ...
                    if (curl_errno($ch)) {
                        ...
                        if (is_array($get_data) && array_key_exists('code', $get_data)) {
                            if ($get_data['code'] == 'ok') {
                                $get_sms_status = 'Success';
                            } else {
                                $get_sms_status = $get_data['message'];
                ...
                case 'SpoofSend':
                    ...
                    $data        = http_build_query($parameters);
                    $gateway_url = $gateway_url.'?sendsms&'.$data;
                ...
                case 'DeliverHub':
                    ...
                    $sendSMS = [
                            ...
                            'SMSSendInputList' => [
                                    [
                                            'FromNumber' => $this->sender_id,
                                            'ToNumber'   => $this->cl_phone,
                                            'Text'       => $this->message,
                                            'Reference'  => time(),
                                    ],
                            ],
        ...
        $check = SMSHistory::where('sender', $sender_id)->where('receiver', $receiver)->where('send_by',
                'receiver')->where('userid', $this->user_id)->first();
        ...
        if ($this->user_id != '0') {
            if ($this->gateway->settings != 'InfoBip' || $this->gateway->settings != 'Twilio' || $this->gateway->settings != 'PlivoPowerpack' || $this->gateway->settings != 'Plivo') {
                if (substr_count(strtolower($get_sms_status), 'success') == 0) {
                    ...
                    if ($sms_cost) {
                        ...
                        if ($area_code_exist) {
                            $format            = $phoneUtil->format($phoneNumberObject,
                        ...
                        $get_operator = Operator::where('operator_setting', $operator_settings)->where('coverage_id',
                                $sms_cost->id)->first();
