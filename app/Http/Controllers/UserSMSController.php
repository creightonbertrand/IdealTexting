...
class UserSMSController extends Controller
{
    ...
    public function postSendBulkSMS(Request $request)
    {
        ...
        // print_r(Carbon::createFromTimeString($request->start_time));
        // print_r(Carbon::createFromTimeString($request->stop_time));
        ...
        if ($request->schedule_sms_status) {
            ...
            $v = \Validator::make($request->all(), [
                'message_type' => 'required', 'remove_duplicate' => 'required', 'country_code' => 'required', 'sms_gateway' => 'required', 'delimiter' => 'required', 'amount' => 'required', 'intaval' => 'required'
            ]);
        ...
        if (isset($results) && is_array($results)) {
            ...
            if (count($results) > 0) {
                ...
                if (isset($final_insert_data) && is_array($final_insert_data) && count($final_insert_data) > 0) {
                    ...
                    if ($request->contact_type == 'phone_book') {
                        if (isset($request->contact_list_id) && is_array($request->contact_list_id) && count($request->contact_list_id)) {
                            $contacts = $request->contact_list_id;
                        }
                    }
            
                    if ($request->contact_type == 'client_group') {
                        $contacts = $request->client_group_id;
                    }
                    ...
                    $campaign = Campaigns::create([
                        ...
                        'intaval' => $request->intaval,  //interval
                        'msg_pause' => $request->msg_pause,
                        'msg_pause_time' => $request->msg_pause_time,
    ...
    // smsTemplates Function Start Here
    //======================================================================
    public function smsReplyTemplates()
    {
        $sms_templates = QuickReply::Where('global', 'yes')->get();
        return view('client.sms-reply-templates', compact('sms_templates'));
    }

    //======================================================================
    // createSmsTemplate Function Start Here
    //======================================================================
    public function createSmsReplyTemplate()
    {
        if (app_config('sender_id_verification') == '1') {
            $all_sender_id = SenderIdManage::where('status', 'unblock')->get();
            $all_ids = [];
        ...
        return view('client.create-sms-reply-template', compact('sender_ids'));
    }

    //======================================================================
    // postSmsTemplate Function Start Here
    //======================================================================
    public function postSmsReplyTemplate(Request $request)
    {
        $v = \Validator::make($request->all(), [
            'template_name' => 'required', 'message' => 'required'
        ]);
        ...
        if ($v->fails()) {
            return redirect('user/sms/create-sms-reply-template')->withErrors($v->errors());
        }
        $exist = QuickReply::where('template_name', $request->template_name)->first();

        if ($exist) {
            return redirect('user/sms/create-sms-reply-template')->with([
                'message' => language_data('Template already exist', Auth::guard('client')->user()->lan_id),
                'message_important' => true
            ]);
        }
        ...
        return redirect('user/sms/sms-reply-templates')->with([
            'message' => language_data('Sms template created successfully', Auth::guard('client')->user()->lan_id)
        ]);
    ...
    // manageSmsTemplate Function Start Here
    //======================================================================
    public function manageSmsReplyTemplate($id)
    {

        $st = QuickReply::find($id);
        if ($st) {
            ...
            return view('client.manage-sms-reply-template', compact('st', 'sender_ids'));
        } else {
            return redirect('user/sms/sms-templates')->with([
                'message' => language_data('Sms template not found', Auth::guard('client')->user()->lan_id),
                'message_important' => true
    ...
    // postManageSmsTemplate Function Start Here
    //======================================================================
    public function postManageSmsReplyTemplate(Request $request)
    {
        $cmd = Input::get('cmd');
        $v = \Validator::make($request->all(), [
            'template_name' => 'required', 'message' => 'required', 'status' => 'required'
        ...
        if ($v->fails()) {
            return redirect('user/sms/manage-sms-reply-template/' . $cmd)->withErrors($v->errors());
        }

        $st = QuickReply::find($cmd);
        ...
        if ($st) {
            if ($st->template_name != $request->template_name) {
                ...
                if ($exist) {
                    return redirect('user/sms/manage-sms-reply-template/' . $cmd)->with([
                        'message' => language_data('Template already exist', Auth::guard('client')->user()->lan_id),
                        'message_important' => true
                    ]);
                }
    ...
    // deleteSmsTemplate Function Start Here
    //======================================================================
    public function deleteSmsReplyTemplate($id)
    {
        $appStage = app_config('AppStage');
        if ($appStage == 'Demo') {
            return redirect('user/sms/sms-reply-templates')->with([
...
// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function getSmsHistoryData(Request $request)
    {
        ...
        $sms_history = SMSHistory::whereIn('userid',$reseller_ids)->orderBy($get_search_column, $short_by);
        ...
        return Datatables::of($sms_history)
            ...
            ->filter(function ($query) use ($request) {
                ...
                if ($request->has('client')) {
                    ...
                    $client = Client::where('fname', 'like', "%{$request->get('client')}%")->orWhere('lname', 'like', "%{$request->get('client')}%")->first();
                    if ($client){
                        $query->where('userid', $client->id);
                    }
                }
...
//======================================================================
    ...
    public function campaignReports()
    {
        ...
        // dd($clients);
        return view('client.campaign-reports', compact('clients'));
    }
    ...
    public function getCampaignReports(Request $request)
    {
        ...
        return Datatables::of($campaigns)
            ->addColumn('action', function ($campaign) {
                $button = '
                <a class="btn btn-success btn-xs" href="' . url("user/sms/manage-campaign/$campaign->id") . '" ><i class="fa fa-line-chart"></i> View </a>';
                if($campaign->status != 'Delivered') {
                    if(in_array($campaign->status, ['Running', 'In Progress'])) {
                        $button .= '
                        <a href="#" id="' . $campaign->id . '" class="cpause btn btn-xs btn-warning"><i class="fa fa-trash"></i> Pause Campaign</a>';
                    }else {
                        $button .= '
                        <a href="#" id="' . $campaign->id . '" class="cresume btn btn-xs btn-warning"><i class="fa fa-trash"></i> Resume Campaign</a>';
                    }
                }
            ...
            ->addColumn('throttling', function ($campaign) {
                return "$campaign->amount Message(s) Every $campaign->intaval Minute(s)";
 ...
 //======================================================================
    ...
    public function postUpdateCampaign(Request $request)
    {
        ...
        if ($campaign) {
            ...
            if(!empty($request->amount) && !empty($request->intaval)) {
                $campaign->amount = $request->amount;
                $campaign->intaval = $request->intaval;
            }
            $campaign->message = $request->message;
            $campaign->sender = $request->sender;
            // Prince Awesome
    ...
    public function chatBox(Request $request)
    {
        ...
        return view('client.chat-box', compact('sms_history', 'sms_count', 'sms_templates'));
    }
    ...
    public function viewChatReports(Request $request)
    {
        ...
        if ($inbox_info) {
            ...
            return response()->json([
                ...
                'data' => $sms_inbox,
                'sms_id' => $id,
                'tags' => $inbox_info->tags->pluck('name')
            ]);
    ...
    {
        $cmd = $request->sms_id;
        $message = $this->process($request->message);
    ...
    public function addChatTag(Request $request)
    {
        ...
        // dd($contact);
        if ($sms_history) {
            $contact = ContactList::where('phone_number', $sms_history->receiver)->first();
            $sms_history->tag($request->tag);
    ...
    public function removeChatTag(Request $request)
    {
        ...
        if ($sms_history) {
            ...
            $contact = ContactList::where('phone_number', $sms_history->receiver)->first();
            $sms_history->untag($request->tag);
            $contact->untag($request->tag);
    ...
    //======================================================================
    // assignBulkCampaign Function Start Here
    //======================================================================
    public function assignBulkCampaign(Request $request)
    {
    ...
    public function phoneNumber(Request $request)
    {
        ...
        if ($request->method() == 'POST') {
            ...
            if ($v->fails()) {
                return redirect('user/sms/phone-number')->withErrors($v->errors());
            }
            ...
            try {
                ...
                if($request->number_type == 'tollfree'){
                    $local = $Signalwire->availablePhoneNumbers("US")->tollFree->read(["areaCode" => $request->area_code], 100);
                }elseif($request->number_type == 'local') {
                    $local = $Signalwire->availablePhoneNumbers("US")->local->read([
                ...
                return redirect('user/sms/phone-number')->withErrors($e->getMessage());
            }


            if(count($local) == 0) {
                return redirect('user/sms/phone-number')->withErrors('No number available  for the specified area code');
            }
        ...
        return view('client.phonenumber', compact('local'));
    }
    ...
    public function getPhoneNumber($number)
    {
        ...
        try {
            $clients_id = json_encode(array(Auth::guard('client')->user()->id), true);
            $gateway = SMSGateways::where('settings', 'Signalwire')->first();
            $gateway_credential = SMSGatewayCredential::where('gateway_id', $gateway->id)->where('status', 'Active')->first();
            ...
            return redirect('user/sms/phone-number')->with([
                'message' => $e->getMessage(),
                'message_important' => true
            ]);
    ...
    public function postPhoneNumber(Request $request)
    {
        ...
        return redirect('user/sms/sender-id-management')->with([
            'message' => "Numbers Assign successfully"
        ]);
    }


    //======================================================================
    // pauseCampaign Function Start Here
    //======================================================================
    public function pauseCampaign($id)
    {
       ...
       //======================================================================
    // resumeCampaign Function Start Here
    //======================================================================
    public function resumeCampaign($id)
    {
        ...
        if ($campaign) {
            ...
            $campaign->run_at = date('Y-m-d H:i:s');
            $campaign->save();
    ...
    public function autoTags()
    {
        ...
        $sms_templates = AutoTags::all();
        return view('client.auto-tags', compact('sms_templates'));
    }

    //======================================================================
    // createAutoTags Function Start Here
    //======================================================================
    public function createAutoTags()
    {

        $self = 'auto-tags';
        

        return view('client.create-auto-tags');
    }

    //======================================================================
    // postAutoTags Function Start Here
    //======================================================================
    public function postAutoTags(Request $request)
    {
        ...
        $v = \Validator::make($request->all(), [
            'name' => 'required', 'keyword' => 'required', 'tags' => 'required'
        ]);

        if ($v->fails()) {
            return redirect('user/sms/create-auto-tags')->withErrors($v->errors());
        }


       
        $exist  = AutoTags::where('title', $request->name)->first();


        if ($exist) {
            return redirect('user/sms/create-auto-tags')->with([
                'message' => "Rule aleady exist",
                'message_important' => true
            ]);
        }
        ...
        $st                = new AutoTags();
        // $st->cl_id         = '0';
        $st->title = $request->name;
        $st->tags          = $request->tags;
        $st->keyword       = $request->keyword;
        $st->save();
    ...
    // manageSmsTemplate Function Start Here
    //======================================================================
    public function manageAutoTags($id)
    {
        ...
        if ($st) {
            ...
            return view('client.manage-auto-tags', compact('st'));

        } else {
            return redirect('user/sms/auto-tags')->with([
    ...
    // postManageSmsTemplate Function Start Here
    //======================================================================
    public function postManageAutoTags(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('sms/manage-auto-tags/' . $cmd)->withErrors($v->errors());
        }

        $st = AutoTags::find($cmd);

        if ($st) {
            if ($st->title != $request->name) {
                ...
                if ($exist) {
                    return redirect('user/sms/manage-auto-tags/' . $cmd)->with([
    ...
    // deleteSmsTemplate Function Start Here
    //======================================================================
    public function deleteAutoTags($id)
    {
