...
class SMSController extends Controller
{
    ...
    public function sendBulkSMSU()
    {
        ...
        $keyword       = Keywords::where('user_id', 0)->get();
        $clients           = Client::where('status', 'Active')->get();
        $sender_ = SenderIdManage::all();
        
        $sender_id = [];
        foreach ($sender_ as $s) {
            
            $id = json_decode($s->cl_id)[0];
            
            $client = Client::where('status', 'Active')->find($id);
            //print_r($client->lname);
            $sender_id[$id.'-'.$s->sender_id] = '('.$s->sender_id.')'.' '.$client->fname.' '.$client->lname;
        }
        
        $schedule_sms = false;

        return view('admin.send-bulk-sms-user', compact('client_group', 'gateways', 'sms_templates', 'phone_book', 'schedule_sms', 'country_code', 'keyword', 'sender_id', 'clients'));
    ...
    public function smsReplyTemplates()
    {
        ...
        return view('admin.sms-reply-templates', compact('sms_templates'));
    }
    ...
    public function createSmsReplyTemplate()
    {
        ...
        return view('admin.create-sms-reply-template');
    }
    ...
    public function postSmsReplyTemplate(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('sms/create-sms-reply-template')->withErrors($v->errors());
        }
        ...
        if ($exist) {
            return redirect('sms/create-sms-reply-template')->with([
                'message' => language_data('Template already exist'),
                'message_important' => true
            ]);
        }
        ...
        return redirect('sms/sms-reply-templates')->with([
            'message' => language_data('Sms template created successfully')
        ]);
    ...
    public function manageSmsReplyTemplate($id)
    {
        ...
        if ($st) {
            ...
            return view('admin.manage-sms-reply-template', compact('st'));
    ...
    public function postManageSmsReplyTemplate(Request $request)
    {
        ...
        if ($st) {
            if ($st->template_name != $request->template_name) {
                ...
                if ($request->set_global == 'yes') {
                    $exist = QuickReply::where('template_name', $request->template_name)->where('global', 'yes')->first();
                } else {
                    $exist = QuickReply::where('template_name', $request->template_name)->where('global', 'no')->first();
                }

                if ($exist) {
                    return redirect('sms/manage-sms-reply-template/' . $cmd)->with([
                        'message' => language_data('Template already exist'),
                        'message_important' => true
                    ]);
                }
    ...
    public function postAutoTags(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('sms/create-auto-tags')->withErrors($v->errors());
        }
        ...
        if ($exist) {
            return redirect('sms/create-auto-tags')->with([
    ...
    public function manageAutoTags($id)
    {
        ...
        if ($st) {
            ...
            return redirect('sms/auto-tags')->with([
                'message' => language_data('Sms template not found'),
                'message_important' => true
            ]);
        }
    ...
    public function postManageAutoTags(Request $request)
    {
        ...
        if ($st) {
            if ($st->title != $request->name) {
                ...
                if ($exist) {
                    return redirect('sms/manage-auto-tags/' . $cmd)->with([
...
//======================================================================
    ...
    public function postNewKeyword(Request $request)
    {
        ...
        $keyword = Keywords::create([
            ...
            'validity' => $validity,
            'validity_date' => $nd,
            'maximum_delay' => $request->maximum_delay,
            'minimum_delay' => $request->minimum_delay
        ]);
        ...
        return Datatables::of($campaigns)
            ->addColumn('action', function ($campaign) {
                if($campaign->status == 'Inactive') {
                    return '
                <a class="btn btn-success btn-xs" href="' . url("sms/manage-campaign/$campaign->id") . '" ><i class="fa fa-line-chart"></i> View</a>
                <a href="#" id="' . $campaign->id . '" class="cstart btn btn-xs btn-primary"><i class="fa fa-plane"></i> Send </a>
    ...
    // postSendBulkSMS Function Start Here
    //======================================================================
    public function postSendBulkSMSU(Request $request)
    {
        $self = 'send-bulk-sms';
        if (Auth::user()->username !== 'admin') {
            $get_perm = Permission::permitted($self);
        ...
        if (false) {
            ...
            $redirect_url = 'sms/send-schedule-sms-user';
        } else {
            $v = \Validator::make($request->all(), [
                'sms_gateway' => 'required', 'message_type' => 'required', 'remove_duplicate' => 'required', 'country_code' => 'required', 'delimiter' => 'required'
            ]);

            $redirect_url = 'sms/send-sms-user';
        }
        ...
        $send_d = explode('-', $request->send_id);
        ...
        if (isset($results) && is_array($results)) {
            ...
            if (count($results) >= 0) {
                ...
                if ($campaign) {
                    ...
                    if ($campaign_list) {
                        return redirect($redirect_url)->with([
                            'message' => 'SMS added on behave of user'
                        ]);
                    }
